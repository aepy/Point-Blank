...
void cMenu::ShowMenu(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if(SHOW){
		...
		NO=0;
		acat("[+] Direct3D","Open|Close",&opt.d3d.main);
		...
		DrawGameTextPixelFlag(X+(WIDTH)/4,MC_ITEMY1(NO)+(F_SPC/2)+1,C_INFO,DT_CENTER|DT_SHADOW,"After MT");
		// Draw FrameRate
		...
		DrawGameTextPixelFlag(MC_FCTR(X,MC_MAXX),MC_ITEMY3(NO)+(F_SPC/2),C_FOOTER,DT_CENTER|DT_SHADOW,"[www.Sadacyber419.blogspot.com]");
