...
public:
	...
	void Hendy(int x, int y, DWORD color, const char *fmt, ...);
...
{	
	...
	if (Health < 10) HPcol = RED; // Jika HP Di Bawah 10 = Color
...
void dxESP::Hendy(int x, int y, DWORD color, const char *fmt, ...)
{
	RECT FontPos = { x, y, x + 120, y + 16 };
	char buf[1024] = {'\0'};
	va_list va_alist;
