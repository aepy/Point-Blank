...
if(YoWorlToScreenGa(pDevice,OnWorld,OnScreen)){
						int x = ( OnScreen.x);
							int y = ( OnScreen.y);
							int Color = RED;
							FillRGB(x,y,6,true,Color,pDevice);
							FillRGB(x-10, y, 7, 1,RainbowTextTest,pDevice);//Left line
							FillRGB(x+4, y, 7, 1,RainbowTextTest,pDevice);//Right line
							FillRGB(x, y-10, 1, 7,RainbowTextTest,pDevice);//Top line
							FillRGB(x, y+4, 1, 7,RainbowTextTest,pDevice);//Bottom line
							...
							PrintTextv2("TARGET LOCKED !",OnScreen.x,OnScreen.y,RainbowTextTest, NHT3XT);
