...
#pragma message("Last Edited By : Fahmi Andrian")
#pragma message("Author : JRC Hack")
#pragma message("JRC  Hack Revolution")
#pragma message("Copyright ©2015-2016 WWW.GAMEJRC.BLOGSPOT.COM")
...
#define MCOLOR_TITLE	D3DCOLOR_ARGB(255, 255, 000, 000)			// Warna Judul
...
#define Menu_special		Green//garis menu
#define TitleText			Black//title text
#define Colors				White// title tex3
...
void Item(CHAR *text, CHAR **opt, INT &val, INT maxvalue, LPDIRECT3DDEVICE9 pDevice)
{
	...
	if(Menu && NHT3XT)
	{
		...
		if(val > 0)
		{
			...
			FillGradients (menux + 1,menuy + 22 + (qntHacks * 20),14,170,Green,Cyan,pDevice);//indikator tombol fiturjika on
		}
		if(val == 0)
		{
			//Rectangle(menux + 2, menuy + 22 + (qntHacks * 20), 12, 12, Green, pDevice);//bingkai indikator fitur
			//FillGradients (menux + 1,menuy + 22 + (qntHacks * 20),14,170,Purple,Purple,pDevice);//indikator tombol fiturjika off
		}

		Color1 = White;
		Color2 = (bVar)?Red:White;

		PrintText(NHT3XT, menux + 1, menuy + 20 + (qntHacks * 20), TLeft, Color1, text);
...
void Folder(CHAR *text, CHAR **opt, INT &val, INT maxvalue, LPDIRECT3DDEVICE9 pDevice)
{
	...
	if(Menu && NHT3XT)
	{
		...
		if(val == 0)
		{
			...
			//FillGradients (menux + 1,menuy + 22 + (qntHacks * 20),14,170,Black,Cyan,pDevice);//indikator tombol fiturjika off
		}

		Color1 = Blue;
		Color2 = (bVar)?Blue:Blue;
...
//		PrintTextv2(text,x+5, y+30 +(MenuSelection * 20), RainbowTextTest, NHT3XT);
}
  ...
  for (i=0; i<Active; i++) {
       ...
       if (i==Mpos){
        if(RainbowTexts!=100)
		   ...
		   texty=(val)?MCOLOR_CURRENT:MCOLOR_CURRENT;
	   }
...
void DrawMenuNav(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (GetAsyncKeyState(VK_UP)&1) {
		...
		if (GetAsyncKeyState(VK_LBUTTON)&1 && *D3MENU[Mpos].var > 0                      ) dir=-1;
		
		if (GetAsyncKeyState(VK_RBUTTON)&1 && *D3MENU[Mpos].var < (D3MENU[Mpos].maxvalue-1)) dir=1;
		if (dir) {
			*D3MENU[Mpos].var += dir;
