...
{
    DWORD g_pGameContext   = *(DWORD*)ResultBasePlayer2; //"bool __thiscall CGameCharaBase::RGC-NET"...
...
{
//PrintText ("HAJAR !!!", OnScreen.x,OnScreen.y,RED, pFonts);
{
int x = ( OnScreen.x);
int y = ( OnScreen.y);
int Color = RED;
...
//DrawLine(683,384,OnScreen.x,OnScreen.y,1,RED);///Line Bullet
...
NHFunctionAimX((PBYTE)OFS_Telekill,(DWORD)i3CollideeLine_SetEnd);
...
{
	if(ShootReal == false){
		MakeNFCZ((PBYTE)ResultGetButton,(DWORD)MouseHook);
		ShootReal = true;
	}
	if(Nh_Teknology9==1){
		if(GetPlayerTarget(MyCharaIndexNFCZ()) > NULL)Shot = true;
		else Shot = false;
	}
}
		//======================================================//
