/*cFunction D3D9 PBHacks Last Edit By Angga Reviza*/
#include <windows.h>
#include <winternl.h>

/*---------------------------- Declaration D3DXVECTOR3 ----------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------------------*/
bool WriteMemory(DWORD dwAddr, void *vVal, int size)
	{
		DWORD OldProtect;
		if(VirtualProtect((void*)dwAddr, size, 0x40, &OldProtect))
		{
			memcpy((void*)dwAddr, vVal, size);
			return VirtualProtect((void*)dwAddr, size, OldProtect, &OldProtect) != 0;
		}
		return false;
	}

/*-------------------------------------------------------------------------------------------------------------------*/
bool WritePointer(DWORD dwPtrBase, void* vVal, int size, int level, ...)
	{
		int i, offset;
		DWORD dwTemp = 0;
		...
		{
			dwPtrBase = *(PDWORD)dwPtrBase;
			for(i = 0; i < level; i++)
			{
				offset = va_arg(vaarg, int);
				if(IsBadReadPtr((void*)(dwPtrBase + offset), 4))
				{
					va_end(vaarg);
					return false;
				}
				dwTemp = dwPtrBase + offset;
				dwPtrBase = *(PDWORD)(dwPtrBase + offset);
			}
		}

		va_end(vaarg);
		return WriteMemory(dwTemp, vVal, size);
	}
