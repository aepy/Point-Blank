/*cMenu.h D3D9 PBHacks Last Edit By Cyber Crekerz*/
#include <windows.h>
...
#define			COLORTEXT		D3DCOLOR_ARGB(255, 000, 255, 000)
...
{
/*------------------------------------------- Declaration Title ------------------------------------------------------*/
	AddText("Sadam Cyber Hack "   ,"Update  @2016");
	AddText("Admin"			    ,"Muhammad Sadam S");
/*------------------------------------------- Declaration D3D ------------------------------------------------------*/
	AddItem("Menu Wallhack"			, Moptfolder, &Direct_D3D		   , 2, MENUFOLDER);
	if(Direct_D3D)
	{
	  AddText("Date"												, GetDate());
	  AddText("Time"												, GetTime());
	  AddItem("Wallhack"		    , Moptonoff, &WHClean			, 2,  MENUCAT);// Khusus PB Indo
//	  AddItem("Wallhack"		    , Moptonoff, &WHRobot			, 2,  MENUCAT); // Digunakan Untuk PB Luar
	  AddItem("Champ Rainbow"		, Moptonoff, &Chams				, 2,  MENUCAT); // Khusus PB Indo
//	  AddItem("NoFog & Smoke"		, Moptonoff, &DontSmoke			, 2,  MENUCAT);
	...
	if(Direct_WEAPON)
	{
	  ...
	  AddItem("Rapid Fire"		    , Moptonoff,  &frpid			, 2,  MENUCAT);
		...
		AddText("Thank's Telah Order Cheat Kami"   ,"TRIAL 3 HARI");
}
